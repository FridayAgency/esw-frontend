...
import { NewsArticle, PagePanelsPagePanelsHeroHeaderLayout, Post } from "@/types/graphql";
...
const HeroHeader: React.FC<HeroHeaderProps> = ({ panel }) => {
  const { copy, title, featuredPost } = panel || {};

  const article = featuredPost?.edges?.[0]?.node as Post | NewsArticle | undefined;
  ...
  return (
    <section className={styles["hero-header"]}>
      ...
      <Container className={styles["hero-header__container"]}>
        ...
        {article && (
          <Link href={article.uri ?? ""} className={styles["hero-header__article"]}>
            <div className={styles["hero-header__article__category"]}>
              {article.__typename === "Post" ? "Blog" : "News"}
            </div>
            <div className={styles["hero-header__article__content"]}>
              <div className={styles["hero-header__article__title"]}>{article.title}</div>
              <svg
