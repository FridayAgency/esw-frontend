...
const OpenRolesPage = () => {
  ...
  useEffect(() => {
    const fetchOpenRoles = async () => {
      setLoading(true);
      try {
        const fetchCareerOpenRolesResponse = await fetch(
          `https://boards-api.greenhouse.io/v1/boards/esw/jobs/?content=true`,
        );
    ...
    <>
      <section className={styles["careers-open-roles"]}>
        {/* <HeroHeaderSimple
          panel={{
            title: "Open Roles",
            copy: "Our team is growing across all of our international offices and we’re looking for great people to join us!",
          }}
        ></HeroHeaderSimple> */}
        <Container flush narrow className={styles.careersOpenRoles}>
          ...
          <div className={styles["careers-open-roles__filter"]}>
            <form onSubmit={handleSubmit} className={styles["careers-open-roles__filter__form"]}>
              ...
              <button type="submit" disabled={loading} className={styles["careers-open-roles__filter__form__button"]}>
